...
using Fcs.Identity.FunctionalTests.Configurations;
using Fcs.Identity.FunctionalTests.Support;
using Fcs.Identity.WebApi.Models;
using FluentAssertions;
using Reqnroll;

namespace Fcs.Identity.FunctionalTests.StepDefinitions;
...
public sealed class AuthEndpointStepDefinitions
{
    ...
    public async Task Then_ARespostaDeveConterOTokenDeAcesso()
    {
        ...
        cookies.Should().Contain(cookie => cookie.Contains("fcs_access_token=access-token"));
    }
    ...
    public async Task Then_ARespostaDeveConterONovoTokenDeAcesso()
    {
        ...
        payload.Data.TokenType.Should().Be("Bearer");
        _response!.Headers.TryGetValues("Set-Cookie", out var cookies).Should().BeTrue();
        cookies.Should().Contain(cookie => cookie.Contains("fcs_access_token=new-access-token"));
    }
